...
export default function Brands() {
    ...
    const handleDeleteBrand = async (id: number) => {
        ...
        try {
            ...
            fetchBrands(page, limitData); // Gọi lại danh sách sau khi xóa
        } catch (error) {
    ...
    const handlePageClick = (selectedItem: { selected: number }) => {
        fetchBrands(selectedItem.selected + 1, limitData);
    };

    const handleSearch = async (value: string) => {
        if (!value.trim()) {
            // Nếu rỗng thì load lại toàn bộ danh sách
            fetchBrands(1, limitData);
            return;
        }

        try {
            const res = await axios.get(`/brands/search`, {
                params: {
                    keyword: value,
                },
            });
            if (Array.isArray(res.data.data)) {
                ...
                // Nếu data không phải mảng thì reset state brands về mảng rỗng
                setBrands([]);
            }
            // setBrands(res.data.data);
