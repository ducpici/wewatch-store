...
export default function Order() {
  ...
  const [currentStateFilter, setCurrentStateFilter] = useState<number[] | null>(
    null
  );
  ...
  const handlePressEdit = async (order_id: bigint, state: any) => {
    if (state !== 0) {
      toast.warning("Chỉ có thể cập nhật khi đơn hàng đang chờ xác nhận!");
  ...
  return (
    <div className="container md:max-w-6xl m-auto px-2">
      ...
      <div className="max-w-6xl mx-auto bg-white">
          ...
          <div className="flex border-b border-gray-200 w-full">
            {tabs.map((tab) => (
              <button
                ...
                disabled={loading}
                className={`cursor-pointer py-2 px-3 md:px-6 md:py-4 text-sm whitespace-nowrap font-medium border-b-2 transition-colors ${
                  activeTab === tab.label
        ...
        {/* Orders */}
        {!loading && orders.length > 0 && (
          <div className="max-h-[700px] overflow-y-auto divide-y divide-gray-200 custom-scrollbar space-y-2">
            {orders.map((order) => (
              <div
                key={order.id}
                className="p-3 md:p-6 cursor-pointer hover:bg-gray-50 transition duration-200 ease-in-out border border-gray-200 ronded"
                onClick={() => navigate(`/don-hang/chi-tiet/${order.id}`)}
              >
                {/* Order Header */}
        ...
        {hasMore && !loading && (
          <div className="text-center py-4">
            <button
              onClick={() => fetchOrders(page + 1, limitData, true)}
