...
const defaultFilters: FilterItem[] = [
  ...
  },
  {
    label: "Loại máy",
    key: "movement_type",
    options: ["Quartz (Pin)", "Eco-Drive (Năng lượng ánh sáng)", "Automatic"],
  },
];
...
const ProductFilters: React.FC<ProductFiltersProps> = ({
  ...
  const renderDropdown = (key: string, options: string[]) => {
    ...
    if (isMobile) {
      return (
        <div
          className="fixed inset-0 z-1 bg-black/30 flex items-start justify-center"
      ...
      <div className="absolute right-0 left-auto md:left-0 md:right-auto bg-white shadow-lg border mt-1 z-20 rounded text-sm min-w-max max-h-[60vh] overflow-y-auto">
        {options.map((option) => (
          <div
            ...
            onClick={() => handleSelect(key, option)}
            className={`px-4 py-2 hover:bg-gray-100 cursor-pointer whitespace-nowrap ${
              selectedFilters[key] === option ? "bg-blue-50 text-blue-600" : ""
            }`}
          >
            {option}
          </div>
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-2 relative z-1 ${className}`}
    >
      {/* Bộ lọc tổng */}
      {showAdvancedFilter && (
        ...
        <div key={key} className="relative">
          <button
            ...
            className={`border px-3 py-1 rounded text-sm hover:bg-gray-50 transition-colors flex items-center gap-1 ${
              selectedFilters[key] ? "bg-blue-50 border-blue-300" : ""
            }`}
          >
            {label} {selectedFilters[key] && `: ${selectedFilters[key]}`}
        ...
        <div className="relative ml-auto">
          <button
            ...
            className={`border px-3 py-1 rounded text-sm hover:bg-gray-50 transition-colors cursor-pointer ${
              selectedFilters["sort"] ? "bg-blue-50 border-blue-300" : ""
            }`}
          >
            Sắp xếp
